"""Generate all AOML demo figures v2 — with fixes:
1) All speeds in knots (1 m/s = 1.9438 kt)
2) Erin peak wind map: filter to SMAP or EOS-6, include best track + sensor max in caption
3) Ship track: jet colorbar
4) Coverage maps: 6-hour panels
5) Buoy time series: NDBC 41049 at actual location, black X markers
"""
import sys
import glob
import os
...
from mwow_tools import (
    ...
    plot_sensor_coverage, plot_wind_map,
...
def gen_timeseries_with_buoy():
    """Time series at NDBC 41049 (27.5N, 62.3W) with buoy data overlay."""
    ...
    buoy_speed_kt = buoy_valid['wind_speed_ms'].values * MS_TO_KT
    buoy_time = buoy_valid['time'].values
    ...
    # Buoy data as black X markers (hourly average for clarity)
...
def gen_wind_map_erin():
    ...
    for sensor_id, sensor_label in [(2, 'EOS-6'), (5, 'SMAP')]:
        ...
        # Pick orbit with highest max wind
        ...
        # Representative time from valid pixels
...
def gen_ship_track():
    """Ship track: NOAA Pisces (Apr 7-8 2026), EPI data, all sensors labeled."""
    print('=== Ship Track (NOAA Pisces, Apr 7-8 2026, EPI MWOW) ===')

    ship_all = pd.read_csv(os.path.join(TRACK_DIR, 'noaa_pisces_apr2026.csv'),
                           parse_dates=['time'])

    # Select clean eastward transit segment (points 25-31, Apr 7 06:00 – Apr 8 18:00)
...
def gen_coverage_6hr_panels(root, suffix, title_prefix):
    """Sensor coverage as 4 x 6-hour panels."""
    print(f'=== Coverage 6hr Panels ({title_prefix}) ===')

    region = {'lat_center': 19.5, 'lon_center': -58.0, 'lat_size': 10.0, 'lon_size': 12.0}

    # Get 1 day (4 files = 4 six-hour windows)
    ...
    for i, (f, ax, tlabel) in enumerate(zip(files, axes_flat, time_labels)):
        ds = open_mwow_files([f])
        plot_sensor_coverage(ds, region=region, qi_max=2,
                             title=f'Aug 16, 2025  {tlabel}',
                             ax=ax)
...
if __name__ == '__main__':
    ...
    if section == 'timeseries' or section == 'all':
        gen_timeseries_with_buoy()
    if section == 'windmap' or section == 'all':
        gen_wind_map_erin()
    if section == 'shiptrack' or section == 'all':
        gen_ship_track()
    if section == 'coverage_nonepi' or section == 'all':
        gen_coverage_6hr_panels(DATA_ROOT, 'nonepi', 'Non-EPI')
    if section == 'coverage_epi' or section == 'all':
        gen_coverage_6hr_panels(EPI_ROOT, 'epi', 'EPI')
